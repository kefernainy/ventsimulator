function VentEngine() {
    ...
    */
    ...
    function patientBreathController() {
        ...
        if (machineInspiration && !patientInspiration && ((60/patientFrequency-patientBreathTimer)< 60/(8*patientFrequency)) ){
    ...
    function machineBreathController() {
        ...
        else if (BreathTarget == "PC") { //time cycle
            if ((machineInspiration) && (machineBreathTimer > inspiratoryTimeSet)) {
                endOfInspiration();

            }
        }
        else if (BreathTarget == "PS") { //flow cycle
        ...
        //flow trigger
        if ( (triggerType === "Flow") && (-1 * flowRequiredForPressure < setFlowTrigger)) {
            machineInspiration = true;
            newBreath = true;
        ...
        if ( (triggerType === "Pressure") && (((lungVolume * compliance) + patientBreathPressure) < (setPressureTrigger + PEEP))) {
            machineInspiration = true;
            newBreath = true;
            volumeMeasured = 0; //reset measured volume
        }
        ...
        //machine trigger
        if (!machineInspiration && (machineBreathTimer >= (60 / machineFrequency)) && (BreathTarget != "PS")) {
            machineInspiration = true;
            newBreath = true;
            volumeMeasured = 0; //reset measured volume
        }

        if (VentMode == "IMV") { //time trigger
    ...
    function patientBreath() {
        ...
        if (patientInspiration) {
            ...
            if (patientBreathTime/patientInspiratoryTimeSet < 0.5){
                patientBreathPressure = maxPatientBreathPressure*2*patientBreathTime/patientInspiratoryTimeSet;
            }else{
                patientBreathPressure = maxPatientBreathPressure*2*(patientInspiratoryTimeSet - patientBreathTime)/patientInspiratoryTimeSet;
            }
    ...
    function machineBreath() {
        ...
        if (BreathTarget === "VC") {
            ...
            while ((staticThoracicPressure + dynamicPressure) < (drivingPressure + PEEP)) {
            ...
            dynamicPressure = flowMeasuredInflow * resistanceOfETT;
            // increase flow until target pressure is reached
            while ((staticThoracicPressure + dynamicPressure) < (drivingSupportPressure + PEEP)) {
                flowMeasuredInflow += 0.01;
                dynamicPressure = flowMeasuredInflow * resistanceOfETT;
            }
            while ((staticThoracicPressure + dynamicPressure) > (drivingSupportPressure + PEEP)) {
    ...
    var expiration = function () {
        ...
        if (doPlateau) {
            ...
            while (((staticThoracicPressure + dynamicPressure) < (PEEP))) {
                ...
                dynamicPressure = flowMeasuredInflow * resistanceOfETT;
                if ((flowMeasuredInflow > 1 + priorVentilatorFlow) || (flowMeasuredInflow > maxInspiratoryFlow)) {
    ...
    function measureParameters() {
        ...
        pressureMeasured = flowMeasuredInflow * resistanceOfETT + lungVolume * compliance + patientBreathPressure;
    ...
    function calculatePatientVentData() {
        ...
        if (newBreath) {
            ...
            var expirationTime = Math.round(machineExpirationTimer / (machineBreathTimer - machineExpirationTimer));
        ...
        if (breathVolumeArray.length > 1) {
            ...
            minuteVentilation = breathRate * breathVolumeArray.reduce(function (previousValue, currentValue, index, array) {
                return previousValue + currentValue;
            }) / breathVolumeArray.length / 1000;
        }
    ...
    thisVent.returnVentDataArray = function () {
        ...
        while (Timer < secondsOfDataToGenerate * ventCounter) //1 seconds of data generated
        {
            ventLoop();
        }
    ...
    this.changeBreathTarget = function (newTarget) {
        ...
        if (newTarget == "VC" || newTarget == "PC" || newTarget == "PRVC" || newTarget == "PS") {
...
addEventListener('message', function (e) {
    ...
    if (e.data.patientBreathDuration) {
        thisVent.changePatientBreathDuration(Number(e.data.patientBreathDuration));
    }
