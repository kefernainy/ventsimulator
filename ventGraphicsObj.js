...
var VentGraphics = function (ventConfigObject) {
    ...
    var numberOfPixelsPerVolume = (height - bottomIndent - heightOfTopBar) / volumePerScreen;
    var numberOfPixelsPerPressure = (height - bottomIndent - heightOfTopBar) / pressurePerScreen;
    var numberOfPixelsPerFlow = (height - bottomIndent - heightOfTopBar) / flowPerScreen / 2;
 ...
 //
    ...
    this.setNumberSecPerScreen = function (numberOfSecondsToShow) {
        if (!isNaN(numberOfSecondsToShow && (numberOfSecondsToShow > 2) && (numberOfSecondsToShow < 31))) {
    ...
    this.initialize = function (elementName) {
        ...
        canvas.outerHTML = "<viewport id=" + elementName + " style = 'width:" + width + "px;height:" + height + "px'></viewport>";
        canvas = document.getElementById(elementName);

        canvas.innerHTML = "<canvas id= 'backgroundLayer-" + elementName + "' style = 'position:absolute; z-index:10;'></canvas>" + "<canvas id= 'ventGraphicsLayer-" + elementName + "' style = 'position:absolute; z-index:20;'></canvas>" + "<canvas id= 'ventDataLayer-" + elementName + "' style = 'position:absolute; z-index:20'></canvas>" + "<canvas id= 'topLayer-" + elementName + "' style = 'position:absolute; z-index:30'></canvas>";


        var canvasBackground = document.getElementById("backgroundLayer-" + elementName);
        var canvasVentGraphicsLayer = document.getElementById("ventGraphicsLayer-" + elementName);
        var canvasVentDataLayer = document.getElementById("ventDataLayer-" + elementName);
        ...
        var Radialgradient = ctxTopLayer.createRadialGradient(75, 50, 5, 90, 60, 300);
...
/////////////
    ...
    this.drawVentBackground = function () {
        ...
        if (!screenValid) {
            ...
            var NumberOfPixelsPerSecond = ((width - leftIndent) / numberSecPerScreen);
            ventBackgroundCacheContext.fillText("Seconds", leftIndent, height - bottomIndent + 10);
            ...
            //Horizontal gradations - just volume for now

            if (display === 'Pressure') {
                var NumberOfPixelsPer10CM = ((height - heightOfTopBar - bottomIndent) * 10) / pressurePerScreen;
                var PresMarker = 0;
                ctxBackgroundLayer.fillText('P', 0, heightOfTopBar + 5);
                for ( i = height - bottomIndent; i > heightOfTopBar; i = i - NumberOfPixelsPer10CM) {
                ...
                var NumberOfPixelsPer100CC = ((height - heightOfTopBar - bottomIndent) * 100) / (volumePerScreen);
                var volMarker = 0;
                ventBackgroundCacheContext.fillText('Vol', 0, heightOfTopBar + 5);
                for ( i = height - bottomIndent; i > heightOfTopBar; i = i - NumberOfPixelsPer100CC) {
                    ventBackgroundCacheContext.moveTo(leftIndent, i);
                    ventBackgroundCacheContext.lineTo(width - leftIndent, i);
                ...
                var NumberOfPixelsPer20CM = ((height - heightOfTopBar - bottomIndent) * 20) / (flowPerScreen) / 2;
                var FlowMarker = 0;
                ventBackgroundCacheContext.fillText('Flow', 0, heightOfTopBar);
                for ( i = ((height - bottomIndent) - (height - heightOfTopBar) / 2); i > heightOfTopBar; i = i - NumberOfPixelsPer20CM) {
                    ventBackgroundCacheContext.moveTo(leftIndent, i);
                    ventBackgroundCacheContext.lineTo(width - leftIndent, i);
                ...
                FlowMarker = 0;
                for ( i = ((height - bottomIndent) - (height - heightOfTopBar) / 2); i < height - bottomIndent; i = i + NumberOfPixelsPer20CM) {
                    ventBackgroundCacheContext.moveTo(leftIndent, i);
                    ventBackgroundCacheContext.lineTo(width - leftIndent, i);
                    ventBackgroundCacheContext.fillText(FlowMarker, 2, i);
                    FlowMarker = FlowMarker - 20;
    ...
    this.drawGraphics = function (ventData) {
        ...
        if ((ventData.breathTarget !== breathTarget) || (ventData.ventMode !== ventMode)) {
        ...
        if (display == "Volume") {
            ...
            newHeight = height - (lungVolumeMeasured * numberOfPixelsPerVolume + bottomIndent);
            ...
            newHeight = height - (pressureMeasured * numberOfPixelsPerPressure + bottomIndent);
            ...
            newHeight = height - (flowMeasured * numberOfPixelsPerFlow + bottomIndent);
        ...
        if (screenTimer === 0) {
            ...
            graphicsCacheContext.clearRect(0,0,graphicsCacheContext.width,graphicsCacheContext.height);
            ctxVentGraphicsLayer.clearRect(0,0,ctxVentGraphicsLayer.canvas.width,ctxVentGraphicsLayer.canvas.height);
    ...
    this.drawTimers = function (ventData){//PeakPressure, PressureMean, PEEPMeasured, BreathRateMeasured, ExhaledTidalVolume, MinuteVentilationMeasured, IoverE, peakPressureAlarm, plateauPressure, expiratoryPressure) {
        ...
        if (!barValid) {
            ...
            maxWidth = width / 8;
            indent = width / 10;
            topbarCacheContext.fillText('P', indent + horizontalDist, verticalDist, maxWidth);
            topbarCacheContext.fillText('I:E', indent + 2 * horizontalDist, verticalDist, maxWidth);
            topbarCacheContext.fillText('f', indent + 3.5 * horizontalDist, verticalDist, maxWidth);
            topbarCacheContext.fillText('V', indent + 4.7 * horizontalDist, verticalDist, maxWidth);
            topbarCacheContext.fillText('V', indent + 7 * horizontalDist, verticalDist, maxWidth);

            indent = indent + 10;
            verticalDist = verticalDist + 10;
            maxWidth = maxWidth / 2;
            topbarCacheContext.font = "10pt Verdana";
            topbarCacheContext.fillText('Peak', indent + horizontalDist, verticalDist, maxWidth);
            topbarCacheContext.fillText('TE', indent + 4.7 * horizontalDist, verticalDist, maxWidth);
            topbarCacheContext.fillText('Etot', indent + 7 * horizontalDist, verticalDist, maxWidth);
            ...
            if (ventMode == "CMV") {
                ...
                if (breathTarget == "VC") {
                    ...
                    topbarCacheContext.fillText('Vt', 10, verticalDist + 35, 2*maxWidth);

                }
                else if (breathTarget == "PC") {

                    topbarCacheContext.fillText('Pt', 10, verticalDist + 30, 2*maxWidth);

                }
                else if (breathTarget == "PRVC") {

                    topbarCacheContext.fillText('Pr', 10, verticalDist + 30, 2*maxWidth);
       ...
       //
        ...
        if (ventData.peakPressureAlarm) {
            ...
            topbarDataCacheContext.fillText("Peak Pressure Alarm", horizontalDist, verticalDist + 12);
        }

        if (ventData.plateauPressure > 0) {
            topbarDataCacheContext.fillStyle = "White";
            topbarDataCacheContext.font = "12pt Arial";
            topbarDataCacheContext.fillText("Plateau Pressure  " + ventData.plateauPressure, horizontalDist, verticalDist + 12);
        }

        if (ventData.expiratoryPressure) {
            topbarDataCacheContext.fillStyle = "White";
            topbarDataCacheContext.font = "12pt Arial";
            topbarDataCacheContext.fillText("Expiratory Pressure  " + ventData.expiratoryPressure, horizontalDist, verticalDist + 12);
        }
        ...
        topbarDataCacheContext.fillText(ventData.peakPressure, indent + horizontalDist, verticalDist, maxWidth);
        topbarDataCacheContext.fillText(ventData.ItoE, indent + 2 * horizontalDist, verticalDist, maxWidth);
        topbarDataCacheContext.fillText(ventData.respiratoryRate, indent + 3.5 * horizontalDist, verticalDist, maxWidth);
        topbarDataCacheContext.fillText(ventData.Vte, indent + 4.7 * horizontalDist, verticalDist, maxWidth);
        topbarDataCacheContext.fillText(ventData.minuteVentilation, indent + 7 * horizontalDist, verticalDist, maxWidth);
